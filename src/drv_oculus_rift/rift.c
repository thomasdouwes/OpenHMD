...
static void
push_hmd (rift_hmd_t *hmd, char *hid_path) {
...
static void
handle_touch_controller_message(rift_hmd_t *hmd,
...
static void
handle_rift_radio_message(rift_hmd_t *hmd, pkt_rift_radio_message *msg)
{
...
static int
getf_touch_controller (rift_device_priv* dev_priv, ohmd_float_value type, float* out)
{
...
static int
getf(ohmd_device* device, ohmd_float_value type, float* out)
{
	rift_device_priv* dev_priv = rift_device_priv_get(device);
	if (dev_priv->id == 0)
 ...
 */
static int
rift_get_led_info (rift_hmd_t *priv)
{
	int first_index = -1;
	unsigned char buf[FEATURE_BUFFER_SIZE];
	int size;
...
static void
init_touch_properties (rift_touch_controller_t *touch, int id, int device_num)
{
...
static rift_hmd_t *
open_hmd (ohmd_driver* driver, ohmd_device_desc* desc)
{
